...
Concrete implementation of the AI agent layer: the `OpenAICompatibleProvider`
that drives both Ollama and the GitHub Copilot Models API via the `openai` SDK,
token-budgeted artifact context shaping, the system prompt, the `/agent/query`
API endpoint, the Agent view UI, and the health-endpoint agent status field.

### Source references

Future changes to this capability MUST reconcile against:

- `openspec/specs/ai-agent/spec.md` — provider-agnostic interface contract
- `.docs/cerebro-open-spec.md` Part II §4.6 (`agent/`)
