...
The dashboard SHALL render a Data view that renders when the artifact contains
...
matching panel set based on the artifact's objective. Each panel (binary,
multiclass, regression, ranking) SHALL be a separate lazy-loaded component.
The view SHALL show a placeholder when no evaluation is present.
